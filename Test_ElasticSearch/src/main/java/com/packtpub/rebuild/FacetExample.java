 ...
 * @Time 下午3:07:08
 */
public class FacetExample extends ElasticSearchBase {
	@Test
	public void testFacet() {
		...
		FacetBuilder facetBuilder2 = statisticalFacet("number1").field(
				"number1");

		SearchResponse response = client.prepareSearch(index).setTypes(type)
				.setQuery(matchAllQuery()).addFacet(facetBuilder)
				.addFacet(facetBuilder2).execute().actionGet();
