...
public enum HbaseUtil {
    MAJOR(HbaseUtil.MAJOR_HBASE_SITE, 0), MAP(HbaseUtil.MAP_HBASE_SITE, 0), TEST(HbaseUtil.TEST_HBASE_SITE, 0);
    ...
    private HbaseUtil(String configPath, int timeOut) {
        ...
        this.hpool = new HTablePool(this.conf, Integer.MAX_VALUE, new CacheHTableFactory(timeOut), null);
    }
     ...
     * @throws IOException
     */
    public HTableInterface getHTableAutoFlush(String tableName) throws IOException {
     ...
     * @throws IOException
     */
    public HTableInterface getHTableNoAutoFlush(String tableName) throws IOException {
    ...
    public byte[][] getUniformSplit(int n) {
        SplitAlgorithm split = new UniformSplit();
        return split.split(n);
    }*/
